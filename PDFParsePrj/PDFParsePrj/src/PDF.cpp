	...
	*str = m_pSource->Read();
	...
	for (pTrailer = m_pTrailer; pTrailer != NULL; pTrailer = pTrailer->pPrev)
	{
		...
		if (pObj != NULL)
		{
            ...
            //查询 StructTreeRoot
            const Dictionary *structTreeRoot = (const Dictionary *)GetObject(((Dictionary *)pObj)->GetValue("StructTreeRoot"));
            const Array *arr  = (const Array *)GetObject(structTreeRoot->GetValue("K"));
//            GetObject(structTreeRoot);
//
